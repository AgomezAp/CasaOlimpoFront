...
export class PacienteNavComponent {
  ...
  setActiveTab(tabId: string): void {
    ...
    const documentoId = this.route.snapshot.paramMap.get('numero_documento');
    this.activeTab = tabId;
    this.tabChange.emit(tabId);
    if (tabId === 'consentimiento') {
      // Usa 'paciente' en singular y verifica que documentoId no sea null
      if (documentoId) {
        this.router.navigate([`/paciente/${documentoId}/consentimiento-info`]);
      } else {
        console.error('Error: Número de documento no disponible');
