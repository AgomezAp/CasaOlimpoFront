...
export class CrearConsultaComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.tipoConsentimientoChangeSub = this.consultaForm
      .get('tipo_consentimiento')
  ...
  guardarConsulta(): void {
    ...
    if (!userEmail) {
      alert(
        'No se pudo obtener el correo del usuario. Por favor inicie sesión nuevamente.'
      );
      return;
    }
    ...
    // Verificar si hay firmas antes de continuar
    if (!this.firmaPaciente || !this.firmaDoctor) {
      alert('Se requieren ambas firmas antes de guardar la consulta');
      return;
    }

    // Verificar si el PDF ya está generado - si no, generarlo primero
    if (!this.pdfGenerado) {
      // Activar indicador de guardando inmediatamente para evitar doble clic
      this.guardando = true;
      ...
      // Configuración similar a la de generarPDF()
      const botonesLimpiar = document.querySelectorAll('.btn-firma');
      botonesLimpiar.forEach((boton) => {
        (boton as HTMLElement).style.display = 'none';
      });
      ...
      html2canvas(elemento, {
        ...
        .then((canvas) => {
          ...
          this.continuarGuardado(userEmail);
        })
        .catch((error) => {
      ...
      // Si el PDF ya está generado, continuar normalmente
      this.guardando = true;
      this.continuarGuardado(userEmail);
    }
  }
  ...
  private continuarGuardado(userEmail: string): void {
  ...
  enviarConsulta(consulta: Partial<Consulta>): void {
    ...
    Object.keys(consulta).forEach((key) => {
      ...
      if (key !== 'consentimiento_info') {
        ...
        formData.append(
          key,
          consulta[key as keyof Partial<Consulta>] as string
        );
      }
    });
    ...
    if (pdfBlob) {
      formData.append(
        'consentimiento_info',
        pdfBlob,
        `consentimiento_${this.numeroDocumento}.pdf`
      );
    }
